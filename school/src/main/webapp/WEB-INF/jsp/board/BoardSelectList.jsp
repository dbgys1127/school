...
<body>
	<div class="container">
		...
		<!-- 목록영역 -->
		<div id="bbs_wrap">
			<div class="total">총 게시물<strong><c:out value="${paginationInfo.totalRecordCount}"></c:out></strong> 건  |  현재페이지
			<strong><c:out value="${paginationInfo.currentPageNo}"></c:out></strong> / <c:out value="${paginationInfo.totalPageCount}"></c:out> </div>
		</div>
		<div class="bss_list">
					...
					<c:forEach var="result" items="${noticeResultList}" varStatus="status">
						<tr class="notice">
							...
							<td class="tit">
								...
								<a href="${viewUrl}"> <c:out value="${result.boardSj}"></c:out> </a>
							</td>
							<td class="writer" data-cell-header="작성자: "><c:out value="${result.frstRegisterId}"></c:out></td>
							<td class="date" data-cell-header="작성일: "><c:out value="${result.frstRegistPnttm}"></c:out></td>
						</tr>
					</c:forEach>
				...
				<!-- 일반글 -->
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr>
							...
							<td class="tit">
								<c:url var="viewUrl" value="/board/select.do${_BASE_PARAM}">
									<c:if test="${result.othbcAt eq 'Y'}">
										<img alt="" src="/asset/BBSTMP_0000000000001/images/ico_board_lock.gif">
									</c:if>
								</c:url>
								<a href="${viewUrl}"> <c:out value="${result.boardSj}"></c:out> </a>
							</td>
							...
							<td class="hits" data-cell-header="조회수: "><c:out value="${result.inquireCo}"></c:out></td>
						</tr>
					</c:forEach>
		...
		<div id="paging">
			<c:url var="pageUrl" value="/board/select.do${_BASE_PARAM}"></c:url>
			<c:set var="pagingParam"><c:out value="${pageUrl}" /></c:set>
			<ui:pagination paginationInfo="${paginationInfo}" type="image" jsFunction="${pagingParam}" />
		</div>
		
		<div class="btn-cont ar">
			<a href="/board/boardRegist.do" class="btn spot"><i class="ico-check-spot" />글쓰기</a>
		</div>
